...
var navWidth=function(){
	...
	if($('.w-logo-img').is(':visible')){
		...
		img.onload = function () {
			...
			$('.row_top .column8').css({'max-width':outWid-logoWid-2});
...
$(function(){
	...
	if($(window).width()>960){
		navWidth();
		nav();
		if($(window).scrollTop()> $('.topLogBoxPc').outerHeight()+$('.topCustom').outerHeight()){
	...
	$(window).scroll(function(){
		if($(window).width()>960){
			if($(this).scrollTop()> $('.topLogBoxPc').outerHeight()+$('.topCustom').outerHeight()){
				$('.w-header-common').css({'position':'fixed','left':0,'right':0,'top':0});
			}else{
				$('.w-header-common').removeAttr('style');
			}
