...
//    当前选中项的所有父节点都显示出来，程序会将点击的li项默认添加.current
	...
	$(".product ul li").each(function(i){
		var xx=$(this).find(".pro_tit").text()
	    $(this).find("a").prepend('<b class="pro_tit1">'+xx+'</b>')
        $(this).find("a").prepend('<div class="item_index"><span class="index_num">0'+(i+1)+'</span><span class="index_line"></span></div>')
		})
	 $(".news ul li").each(function(i){
        $(this).find("a").prepend('<div class="item_index">0'+(i+1)+'</div>')
		})	
		
				$("._aside ul.sidemenu > li.sub1").click(function(){
					$("._aside ul.sidemenu > li.sub1").removeClass("sub1_cur")
					$(this).addClass("sub1_cur")
					})
					
					$(".list-img ul li .img_area").height($(".list-img ul li").width()-2).width($(".list-img ul li").width()-2)
					$(".product .inner ul li .pro_img").height($(".product .inner ul li").width()-4.5).width($(".product .inner ul li").width()-4.5)
