...
$(function(){
	...
	if($(window).width()>960){
		...
		if($(window).scrollTop()> $('.topLogBoxPc').outerHeight()+$('.topCustom').outerHeight()+$('.w-header-common').outerHeight()){
			$('.w-header-common').css({'position':'fixed','left':0,'right':0,'top':0,'border-bottom':'1px solid #eee'});
			$('.w-header-common .col-logo').css({'padding-top':0,'padding-bottom':'5px'});
			$('.w-header-common .w-nav').css({'padding':'17px 0'});
		}else{
			$('.w-header-common').removeAttr('style');
			$('.w-header-common .col-logo').removeAttr('style');
			$('.w-header-common .w-nav').removeAttr('style');
		}
	}else{
		nav_mobile();
	}
